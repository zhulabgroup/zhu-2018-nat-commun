...
wis.dat <- read_rds("Models/wis_dat.rds")
...
ggplot(og.wis.dat) +
  ...
  geom_line(aes(x = as.numeric(sag.cls), y = agb.mod), stat = "summary", fun = mean, col = "red") +
  geom_ribbon(aes(x = as.numeric(sag.cls), y = agb.mod), stat = "summary", fun.min = min, fun.max = max, fill = "red", alpha = .2) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5)) +
  labs(x = "Stand age (yr)", y = expression("Aboveground biomass (Mg" ~ ha^-1 * ")")) +
  facet_wrap(~ft, scales = "free") +
  scale_y_sqrt()
ggsave("Figures/Old-growth data vs model sqrt.pdf", w = 10, h = 5)
...
# map
all.shp <- SpatialPointsDataFrame(
  coords = all.dat[, c("lon", "lat")],
  data = all.dat %>%
    select(ft, cen, sag) %>%
...
og.tm <- tm_shape(all.shp, projection = CRS("+proj=aea +lat_1=20 +lat_2=60 +lat_0=40 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0")) +
  ...
  tm_legend(show = F)
print(og.tm)
tmap_save(og.tm, file = "Figures/Old-growth plot map.pdf", w = 10, h = 10)
...
og.fia.dat <- read_rds("Data/fia_cond_table.rds") %>%
  select(plt = PLT_CN, PROP_BASIS) %>%
  mutate(plt = paste0("US@", as.character(plt))) %>%
  right_join(all.dat %>%
    filter(
      ft %in% c("Douglas-fir", "Pinyon / juniper"),
      src == "fia"
    ),
  by = c("plt")
  ) %>%
  mutate(
    area.m2 = ifelse(PROP_BASIS == "SUBP", # unit = m2
      4 * pi * (7.3152^2), # 1 subplot = 24 ft radius = 7.3152 m radius, 4 subplots in total
      4 * pi * (17.95272^2)
    ), # 1 macroplot = 58.9 ft radius = 17.95272 m radius, 4 macroplots in total
    area.ha = area.m2 * 0.0001
  ) %>% # unit = ha
  select(src, cen, plt, ft, area = area.ha)

og.psp.dat <- read_csv("Data/biomass.pspAfter99.csv") %>%
  mutate(cen = "new") %>%
  select(plt = PlotProv, cen, area = plotsize) %>%
  bind_rows(read_csv("Data/biomass.psp9099.csv") %>%
    mutate(cen = "old") %>%
    select(plt = PlotProv, cen, area = plotsize)) %>%
  right_join(all.dat %>%
    filter(
...
# plot_grid(pal.a, pal.b,
#   nrow = 1,
#   labels = c("a", "b"), vjust = 2.5
# )
# ggsave("Figures/Douglas-fir combined fig.pdf", h = 5, w = 10)
