...
# Read CSV data file
all.dat <- read.csv(paste0(dat.dir, 'All_data.csv'), stringsAsFactors = F) %>% as_tibble() # can't use read_csv

# Regional standardization of climate variables
env.stat <- all.dat %>%
  filter(cen == 'new') %>%
  group_by(ft) %>%
  summarize(tmp.mean = mean(tmp), tmp.sd = sd(tmp),
            ppt.mean = mean(ppt), ppt.sd = sd(ppt)) %>%
  ungroup()

all.dat <- all.dat %>%
  full_join(env.stat) %>%
  mutate(tmp.ctr = tmp - tmp.mean, tmp.std = tmp.ctr/tmp.sd,
...
ft.dat  <- data_frame(id = 1:nlevels(in.dat$ft), ft = levels(in.dat$ft))
...
write_rds(all.dat, paste0(dat.dir, 'all_dat.rds'))
write_rds(env.stat, paste0(dat.dir, 'env_stat.rds'))
write_rds(ft.dat, paste0(dat.dir, 'ft_dat.rds'))
write_rds(jags.dat, paste0(dat.dir, 'jags_dat.rds'))
...
ggsave(paste0(fig.dir, 'Histogram of current plot percentage.pdf'),  w = 10, h = 12)
...
all.dat %>% 
  ...
  labs(x = 'Stand age (yr)', y = 'Percentage of plots') +
  scale_y_continuous(labels = scales::percent)
ggsave(paste0(fig.dir, 'Histogram of past plot percentage.pdf'),  w = 10, h = 12)
...
ggsave(paste0(fig.dir, 'Histogram of current and past plot percentage.pdf'),  w = 10, h = 12)
