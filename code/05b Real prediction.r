# real prediction exercies, DO NOT PUBLISH
library(tidyverse)
library(R2jags)
library(ggmcmc)
theme_set(theme_bw())
...
mcmc.dat <- 'old_jags_2e2.rds' %>% # try 2e2 first, slow
  paste0(mod.dir, .) %>% 
  read_rds() %>% 
  coda::as.mcmc() %>% 
  ggmcmc::ggs()
...
wis.dat <- cur.dat %>% 
  ...
  mutate(mu.hat = beta.int + beta.tmp*tmp.std + beta.ppt*ppt.std,
         k.hat  = gamma.int + gamma.tmp*tmp.std + gamma.ppt*ppt.std,
         agb.hat = mu.hat*sag/(k.hat + sag)) %>% 
  group_by(plt) %>% 
  summarize(mu.mean = mean(mu.hat), mu.med = median(mu.hat), mu.lwr = quantile(mu.hat, .025), mu.upr = quantile(mu.hat, .975),
