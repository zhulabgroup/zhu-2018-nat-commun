...
yhat.tm <- tm_shape(yhat.ras) +
  ...
  tm_layout(asp = 1, # square shape
            title = c('RCP4.5, 2020s', 'RCP4.5, 2050s', 'RCP4.5, 2080s',
                      'RCP8.5, 2020s', 'RCP8.5, 2050s', 'RCP8.5, 2080s'),
            legend.title.size = 1,
            legend.text.size = 0.5,
            legend.position = c("left","bottom"))
print(yhat.tm)
save_tmap(yhat.tm, file = paste0(fig.dir, 'Map of yhat.pdf'), w = 15, h = 10)
...
mu.tm <- tm_shape(mu.ras) +
  ...
  tm_layout(asp = 1, # square shape
            ...
            legend.position = c("left","bottom"))
print(mu.tm)
save_tmap(mu.tm, file = paste0(fig.dir, 'Map of mu.pdf'), w = 15, h = 10)

# # delte mu map
# dmu.tm <- tm_shape(mu.ras) +
#   tm_fill(c('mu_rcp45_2055',     'mu_rcp45_2085',     'delta_mu_rcp45_2085_2055',
#             'mu_rcp85_2055',     'mu_rcp85_2085',     'delta_mu_rcp85_2085_2055'),
#           title = expression('(Mg'~ha^-1*')'),
#           breaks = list(mu.col.brk, mu.col.brk, delta.mu.col.brk,
#                         mu.col.brk, mu.col.brk, delta.mu.col.brk),
#           palette = list(col.vec, col.vec, fields::tim.colors(),
#                          col.vec, col.vec, fields::tim.colors())) +
#   tm_shape(bkgd.poly) + tm_borders('gray80') + # light gray; 'gray20' is dark gray
#   tm_layout(asp = 1, # square shape
#             title = c('RCP4.5, 2050s', 'RCP4.5, 2080s', 'RCP4.5, 2080s - 2050s',
#                       'RCP8.5, 2050s', 'RCP8.5, 2080s', 'RCP8.5, 2080s - 2050s'),
#             legend.title.size = 1,
#             legend.text.size = 0.5,
#             legend.position = c("left","bottom"))
# print(dmu.tm)
# save_tmap(dmu.tm, file = paste0(fig.dir, 'Map of delta mu.pdf'), w = 15, h = 10)
...
k.tm <- tm_shape(k.ras) +
  ...
  tm_layout(asp = 1, # square shape
            ...
            legend.position = c("left","bottom"))
print(k.tm)
save_tmap(k.tm, file = paste0(fig.dir, 'Map of k.pdf'), w = 15, h = 10)
...
r.tm <- tm_shape(r.ras) +
  ...
  tm_layout(asp = 1, # square shape
            ...
            legend.position = c("left","bottom"))
print(r.tm)
save_tmap(r.tm, file = paste0(fig.dir, 'Map of r.pdf'), w = 15, h = 10)
