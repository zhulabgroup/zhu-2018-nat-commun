...
cur.dat <- read_rds(paste0(dat.dir, 'all_dat.rds')) %>% 
  filter(cen == 'new') %>%
...
train.dat <- cur.dat[train.ind, ] %>% 
  write_rds(paste0(mod.dir, 'train_dat.rds'))
test.dat  <- cur.dat[-train.ind, ] %>% 
  write_rds(paste0(mod.dir, 'test_dat.rds'))

# Fit JAGS ----------------------------------------------------------------

# JAGS
ft.dat  <- data_frame(id = 1:nlevels(train.dat$ft), ft = levels(train.dat$ft)) %>% 
  write_rds(paste0(mod.dir, 'train_ft_dat.rds'))
...
monod.jags  <- jags(data = jags.dat,
                             model.file = monod.mod,
                             parameters.to.save = monod.par,
                             n.chains = 2, n.iter = 2e2)
write_rds(monod.jags, paste0(mod.dir, 'spval_jags_2e2.rds'))
