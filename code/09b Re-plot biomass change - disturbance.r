...
old.dat <- read_rds(paste0(mod.dir, 'oos_dat.rds'))
new.dat <- read_rds(paste0(mod.dir, 'wis_dat.rds'))
...
com.shp <- SpatialPointsDataFrame(coords = com.dat[, c('lon', 'lat')], 
...
com.tm <- tm_shape(com.shp, projection = CRS('+proj=aea +lat_1=20 +lat_2=60 +lat_0=40 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0')) +
  ...
  tm_layout(asp = 1)
print(com.tm)
save_tmap(com.tm, file = paste0(fig.dir, 'Map of biomass change.pdf'), w = 10, h = 5)
...
ggsave(paste0(fig.dir, 'Biomass change.pdf'), w = 15, h = 5)
